...
Price <- read.csv("data/tax/Simulated_Price_Variable_NG.csv", header = FALSE) #Hourly Shadow Price Weighted Variable Natural Gas and Carbon Tax
Qualifying_Generators <- read.csv("data/tax/qf_generation.csv", header = FALSE)
...
#Current Scenario 
cur_sce = 'Weighted_Pollution_Tax'
N_years <- 500
...
#They do not own any solar of their own. (Page 17)
...
PPA_cost_per_MWh = 106.2 #These might have to increase
QF_cost_per_MWh = 33.1553 #These might have to increase.
...
for(yr in 1:N_years) {
  ...
  #Generation Cost
  name <- paste0('data/tax/PGE_GEN_cost_', yr-1, '.csv')
  ...
  for (i in 1:ncol(Thermal_cost)) {
    Thermal_cost[,i] <-  Thermal_cost[,i] + thermal_tax[i]
  }
  ...
  #Solar
  name3 = paste0("data/tax/PGE_Solar_",yr-1,".csv")
  ...
  #Wind
  name4 = paste0("data/tax/PGE_Wind_",yr-1,".csv")
